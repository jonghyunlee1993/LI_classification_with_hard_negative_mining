...
class ModelTraining:
    def __init__(self, config, project_name, is_weak=True):
        self.config = config
    ...
    def train_and_eval(self):
        self.trainer.fit(
            self.model_interface, self.train_dataloader, self.valid_dataloader
        )

        test_results = self.trainer.test(
            self.model_interface, self.test_dataloader, ckpt_path="best"
        )
        ...
        torch.save(self.model_interface.model.state_dict(), weight_fname)
