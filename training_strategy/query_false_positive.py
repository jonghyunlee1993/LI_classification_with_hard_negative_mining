...
class QueryFalsePositives:
    def __init__(self, project_name, model, 
                 valid_df, valid_transform, config):
    ...
    def compute_probability(self):
        ...
        for i in tqdm(range(len(self.valid_df))):
            ...
            result = [p_id, fname, pred.detach().cpu().numpy()[0], 
                    prob.detach().cpu().numpy()[0][1], label]
            pred_df.append(result)

        pred_df = pd.DataFrame(pred_df, columns=["p_id", "fname", "pred", "prob", "label"])
    ...
    def query_false_positives(self, threshold=0.1):
        false_positive_df = self.pred_df[(self.pred_df.label == 0) & (self.pred_df.prob >= threshold)].reset_index(drop=True)
        
        for _, line in false_positive_df.iterrows():
            source = line.fname
            dest = source.replace("data/hard_labeled_dataset/",
