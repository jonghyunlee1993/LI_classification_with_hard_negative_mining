...
class HardNegativeMining:
    def __init__(self, project_name, model, valid_transform, config):
    ...
    def get_false_positive_features(self):
        ...
        for ref in self.ref_flist:
            ...
            pooled_ref_feat = (
                torch.mean(
                    ref_hidden.view(ref_hidden.size(0), ref_hidden.size(1), -1), dim=2
                )
                .detach()
                .cpu()
            )
        ...
        with tqdm(total=len(self.ref_feats), desc="Ref Image") as outer:
            with tqdm(total=len(cand_dataloader), desc="Query Images") as inner:
                for i, ref_feat in enumerate(self.ref_feats):
                    ...
                    for batch in cand_dataloader:
                        try:
                            ...
                            cand_feat = (
                                ...
                                .reshape(sample_batch_size, -1)
                                .detach()
                                .cpu()
                            )
                            if self.query_criterion == "L2":
                                dist = (
                                    nn.PairwiseDistance(p=2)(ref_feat, cand_feat)
                                    .detach()
                                    .cpu()
                                    .numpy()
                                    .tolist()
                                ...
                                dist = (
                                    nn.CosineSimilarity()(ref_feat, cand_feat)
                                    .detach()
                                    .cpu()
                                    .numpy()
                                    .tolist()
                    ...
                    self.weakly_feats.extend(
                        result.sort_values("dist", ascending=True)
                        .head(self.number_of_query)
                        .fname.values.tolist()
                    )
                    outer.update()
                    inner.refresh()
    ...
    def query_hard_negatives(self):
        for f in self.weakly_feats:
            shutil.copyfile(
                f"./data/weakly_labeled_dataset/{f.split('/')[-1]}",
                os.path.join(
                    "./results/aug-data_" + self.project_name, f.split("/")[-1]
                ),
            )
