...
class DataManager:
    ...
    def sample_train_dataset(self):
        if self.sample_only_pos:
            ...
            _, self.train_df = train_test_split(
                self.train_df,
    ...
    def log_dataset(self):
        ...
        print(
            f"# of patches\t train: {len(self.train_df)} valid: {len(self.valid_df)} test: {len(self.test_df)}"
        )
        print(
            f"# of labels\t train: {Counter(self.train_df.label.values)} valid: {Counter(self.valid_df.label.values)} test: {Counter(self.test_df.label.values)}"
        )
        print()
